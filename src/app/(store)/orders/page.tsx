...
const Orders = async () => {
  ...
  if (!userId) {
    redirect("/"); // Redirect user if not authenticated
    return null; // Return null to avoid rendering further content
  }
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {order.products?.map((product) => (
                      <div
                        ...
                        className="flec flec-col sm:flex-row sm:items-center sm:justify-between gap-3 py-2 border-b last:border-b-0"
                      >
                        <div className="flex items-center gap-3 sm:gap-4">
                          {product.product?.image && (
                            <div className="relative h-4 w-14 sm:h-16 sm:w-16 flex-shrink-0 rounded-md overflow-hidden">
                              <Image
